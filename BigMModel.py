...
def big_m_model(PUTAWAY=None, PICKING=None):
    ...
    model.Total_Cost_Objective = Objective(sense=minimize, rule=Total_Cost_Objective)



    def BigM_MHE_LOWER(model, i):
    ...
    model.ConstraintSixPutaway = Constraint(model.TIMES, rule=ConstraintSixPutaway)



    def ConstraintSixPicking(model, t):
        Six_expr1 = model.beta_pick[t]
        Six_expr2 = model.gamma * model.alpha_pick
        return (Six_expr1 - Six_expr2 <= 0)

    model.ConstraintSixPicking = Constraint(model.TIMES, rule=ConstraintSixPicking)



    # Constraint Seven
    def ConstraintSeven(model, v, q, s):
    ...
    model.ConstraintSeven = Constraint(model.OMEGA_Q, model.STORES, rule=ConstraintSeven)



    # Constraint Eight
    def ConstraintEight(model, s, q):
        Eight_expr1 = model.tau_sq[s,q]
        return (model.pt - Eight_expr1 <= 0)

    model.ConstraintEight = Constraint(model.STORES, model.FASHION, rule=ConstraintEight)
    ...
    # Constraint Ten
    def ConstraintTenVendor(model, v, q):
        Ten_expr1 = sum(model.rho_vqt[v, q, t] for t in model.TIMES if model.tb <= t <= model.te)
        return (Ten_expr1 - 1 == 0)

    model.ConstraintTenVendor = Constraint(model.OMEGA_Q, rule=ConstraintTenVendor)



    def ConstraintTenStore(model, s, q):
        Ten_expr1 = sum(model.rho_sqt[s, q, t] for t in model.TIMES if model.tb <= t <= model.ty)
        return (Ten_expr1 - 1 == 0)

    model.ConstraintTenStore = Constraint(model.STORES, model.FASHION, rule=ConstraintTenStore)

    # Constraint Ten Prime
    def ConstraintTenPrimeVendor(model, v, q, t):
        if not model.tb <= t <= model.ty:
            return model.rho_vqt[v, q, t] == 0
        else:
            return Constraint.Skip

    model.ConstraintTenPrimeVendor = Constraint(model.OMEGA_Q, model.TIMES, rule=ConstraintTenPrimeVendor)

    def ConstraintTenPrimeStore(model, s, q, t):
        if not model.tb <= t <= model.ty:
            return model.rho_sqt[s, q, t] == 0
        else:
            return Constraint.Skip

    model.ConstraintTenPrimeStore = Constraint(model.STORES, model.FASHION, model.TIMES, rule=ConstraintTenPrimeStore)
    ...
    model.ConstraintEleven = Constraint(model.STORES, model.PRODUCTS, model.TIMES, rule=ConstraintEleven)
    ...
    model.ConstraintThirteen = Constraint(model.STORES, model.FASHION, model.TIMES, rule=ConstraintThirteen)

    # Constraint Fourteen and Fifteen
    def ConstraintFourteen(model, p, t):
    ...
    model.ConstraintFourteen = Constraint(model.PRODUCTS, model.TIMES, rule=ConstraintFourteen)



    # Constraint Sixteen and Seventeen
    def ConstraintSixteen(model,s,p,t):
    ...
    model.ConstraintSixteen = Constraint(model.STORES, model.PRODUCTS, model.TIMES, rule=ConstraintSixteen)



    # Constraint Eighteen
    def ConstraintEighteen(model,s,q,t):
        if model.tb <= t <= model.ty:
            if t == 1:
    ...
    model.ConstraintEighteen = Constraint(model.STORES, model.FASHION, model.TIMES, rule=ConstraintEighteen)



    # Constraints Nineteen
    def ConstraintNineteen(model, s, t):
    ...
    model.ConstraintNineteen = Constraint(model.STORES, model.TIMES, rule=ConstraintNineteen)



    # Constraints Twenty
    def ConstraintTwenty(model, v, t):
    ...
    model.ConstraintTwenty = Constraint(model.VENDORS, model.TIMES, rule=ConstraintTwenty)


    # Constraints TwentyOne
    def ConstraintTwentyOne(model, s, t):
